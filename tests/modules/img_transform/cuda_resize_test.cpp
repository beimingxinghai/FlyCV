...
class CudaResizeTest : public ::testing::Test {
protected:
    void SetUp() override {
        int status = 0;
        ...
        bgra_u8_src = CudaMat(IMG_720P_WIDTH, IMG_720P_HEIGHT, FCVImageType::PKG_BGRA_U8);
        status = read_binary_file(BGRA_1280X720_U8_BIN, bgra_u8_src.data(),
                bgra_u8_src.total_byte_size());
        ASSERT_EQ(status, 0);

        gray_f32_src = CudaMat(IMG_720P_WIDTH, IMG_720P_HEIGHT, FCVImageType::GRAY_F32);
        status = read_binary_file(GRAY_1280X720_F32_BIN, gray_f32_src.data(),
                gray_f32_src.total_byte_size());
        ASSERT_EQ(status, 0);

        bgr_f32_src = CudaMat(IMG_720P_WIDTH, IMG_720P_HEIGHT, FCVImageType::PKG_BGR_F32);
...
TEST_F(CudaResizeTest, GRAYU8InterLinearPositiveInput) {
    CudaMat dst;
...
TEST_F(CudaResizeTest, PkgBGRU8InterLinearPositiveInput) {
    CudaMat dst;
...
TEST_F(CudaResizeTest, PkgBGRAU8InterLinearPositiveInput) {
    CudaMat dst;
...
TEST_F(CudaResizeTest, GRAYF32InterLinearPositiveInput) {
    CudaMat dst;
...
TEST_F(CudaResizeTest, PkgBGRF32InterLinearPositiveInput) {
    CudaMat dst;
...
TEST_F(CudaResizeTest, GRAYU8InterCubicPositiveInput) {
    CudaMat dst;
...
TEST_F(CudaResizeTest, PkgBGRU8InterCubicPositiveInput) {
    CudaMat dst;
